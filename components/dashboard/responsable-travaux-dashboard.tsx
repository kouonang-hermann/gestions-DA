...
export default function ResponsableTravauxDashboard() {
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      ...
      console.log(`🔍 [RESPONSABLE-TRAVAUX-DEBUG] Analyse pour ${currentUser.nom} (ID: ${currentUser.id}):`, {
        totalDemandesAValider: demandesAValider.length,
        projetsAssignes: currentUser.projets
      })

      // Afficher toutes les demandes avec leurs signatures
      demandesAValider.forEach(d => {
        if (d.validationResponsableTravaux) {
          console.log(`  📋 Demande ${d.numero}:`, {
            status: d.status,
            validationResponsableTravaux: d.validationResponsableTravaux?.userId,
            estValideeParMoi: d.validationResponsableTravaux?.userId === currentUser.id,
            demandeur: d.technicienId
          })
        }
      })
      ...
      console.log(`✅ [RESPONSABLE-TRAVAUX-DEBUG] Demandes validées trouvées:`, {
        total: demandesValidees.length,
        numeros: demandesValidees.map(d => d.numero)
      })
      ...
      setStats({
        ...
        // En cours = Demandes validées par moi qui sont encore en traitement
    ...
    switch (type) {
      ...
      case "enCours":
        ...
        return demandesFiltered.filter((d) => 
          d.validationResponsableTravaux?.userId === currentUser.id &&
          !["cloturee", "rejetee", "confirmee_demandeur"].includes(d.status)
        )
